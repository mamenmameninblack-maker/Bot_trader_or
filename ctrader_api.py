...
def is_in_cooldown():
    data = load_cooldown()
    if not data["cooldown_until"]:
        return False
    return datetime.utcnow() < datetime.fromisoformat(data["cooldown_until"])

def activate_cooldown(hours):
    until = datetime.utcnow() + timedelta(hours=hours)
    save_cooldown({"cooldown_until": until.isoformat()})
    print(f"[COOLDOWN] Activé jusqu’à {until} UTC")
...
def notify_closed_position(self, position):
    ...
    try:
        ...
        print("[COOLDOWN] Impossible de lire le profit, cooldown non appliqué.")
        profit = None

    # Cooldown après perte (30 minutes)
    if profit is not None and profit < 0:
        activate_cooldown(hours=0.5)
        print("[COOLDOWN] Activé 30 minutes suite à une perte.")

    result = "🎯 TP TOUCHÉ" if profit and profit > 0 else "❌ SL TOUCHÉ"
